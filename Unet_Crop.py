...
with open('AllDataset2DGaussianNewData36x68.p', 'rb') as fp:
...
# Create Block CNN
def ConvBlock(inputs,out_channels, kernel_size=3, dropout_p= 0):
    x = layers.Conv2D(out_channels, kernel_size)(inputs)
    x = layers.BatchNormalization()(x)
    x = layers.ReLU()(x)
    x = layers.Conv2D(out_channels, kernel_size)(x)
    x = layers.BatchNormalization()(x)
    x = layers.ReLU()(x)
...
def DeconBlock(inputs, skip_connection, out_channels,kernel_size=3):
    ...
    shape = x.shape
    crop = layers.RandomCrop(shape[1],shape[2])(skip_connection)
    Concat = layers.Concatenate(axis=-1)([x , crop])
    x  = layers.Conv2D(out_channels, kernel_size)(Concat)
    x = layers.BatchNormalization()(x)
    x = layers.ReLU()(x)
    x  = layers.Conv2D(out_channels, kernel_size)(x)
    x = layers.BatchNormalization()(x)
...
def Unet(input = layers.Input(shape=(128, 160, 3),name ='Input_Image')):
    ...
    BottleNeck = ConvBlock(x,128)
    # Upconvolution Block
    DeconvBLock1 = DeconBlock( BottleNeck, convblock3, 64)
    DeconvBLock2 = DeconBlock( DeconvBLock1, convblock2, 32)
    DeconvBLock3 = DeconBlock( DeconvBLock2, convblock1, 16)

    #Conv2D layer with filter
    output = layers.Conv2D(filters= 1, kernel_size= 1, activation = 'sigmoid')(DeconvBLock3)
...
for epoch in range(num_epochs):
    ...
    if np.mean(validation_score) > initialize_acuuracy[0]:
        model.save('save model/'+str(np.mean(validation_score))+'Unet_crop__160x128_1st.h5')
        model.save_weights('save model/Unet_crop__Weight_160x128_1st.h5')
...
with open('save model/Unet_crop__learning_rate_1st.p', 'wb') as fp:
    pickle.dump(learning_rate , fp, protocol=pickle.HIGHEST_PROTOCOL)
with open('save model/Unet_crop__loss160x128_1st.p', 'wb') as fp:
    pickle.dump(Training_loss, fp, protocol=pickle.HIGHEST_PROTOCOL)

with open('save model/Unet_crop_validation_accuracy160x128_1st.p', 'wb') as fp:
